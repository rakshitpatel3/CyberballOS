...
export class CyberballScene extends Phaser.Scene {
    ...
    public preload() {
        ...
        //TODO this was a hack fix and may cause problems
        //this.load.start(); 
    }
    ...
    public createHumanPlayer() {
        ...
        const body = this.playerSprite.body as Phaser.Physics.Arcade.Body;
            body.setSize(
            this.playerSprite.frame.realWidth,
            this.playerSprite.frame.realHeight,
            true
        );
    ...
    public createCPU(i: number) {
        ...
        const body = cpuSprite.body as Phaser.Physics.Arcade.Body;
            body.setSize(
            cpuSprite.frame.realWidth,
            cpuSprite.frame.realHeight,
            true
        );
    ...
    public createBall() {
        ...
        //delay physics sizing until texture is guaranteed ready
        this.time.delayedCall(0, () => {
            const body = this.ballSprite.body as Phaser.Physics.Arcade.Body;
    
            const w = this.ballSprite.frame.realWidth;
            const h = this.ballSprite.frame.realHeight;
    
            if (!w || !h) {
                console.error("Ball frame size invalid:", w, h);
                return;
            }
    
            body.setSize(w, h, true);
            console.log("BALL BODY:", body.width, body.height);
        });
    
        this.physics.add.overlap(this.ballSprite, this.playerGroup, (_b, receiver) => {
            if (
                this.cyberballGameController.model.playerHoldingBallId === null &&
                receiver === this.throwTarget
            ) {
